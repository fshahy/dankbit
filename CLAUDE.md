...
- Daily 24H delta=0 line — a distinct, always-**black** `createPriceLine` set (not type-colored like Weekly/Monthly/All), title `Daily 24H`, sourced from `/api/delta-zero-tomorrow/<asset>`. Targets the specific expiry landing on calendar tomorrow (UTC) — computed directly as `today + 1 day` formatted to Deribit's no-leading-zero day convention (e.g. `7JUL26`), not "nearest active expiry" (`MIN(expiration) >= NOW()`), since the latter can still resolve to *today's* not-yet-happened expiry. Restricted to trades from the trailing 24h (`deribit_ts >= NOW() - INTERVAL '24 hours'`), matching Quadrant Gamma's trade-window convention. Replaces the earlier, unused `/api/delta-zero-next/<asset>` endpoint (nearest-expiry + all-time trades, never wired to the frontend).
...
- Footer legend (`#dz-legend`, fixed below the status line): black "S/D", red "Tie-out", violet "Gamma Extrema", blue "Net Call Gamma", orange (`#e65100`) "Net Put Gamma", dark green (`#2e7d32`) "Short Max Price", dark red (`#c62828`) "Long Min Price", black "Daily 24H"
...
- `dankbit.zones.extrema` — Every-4-hours snapshot of two prices per asset: `short_max_price` (where the Shorts payoff curve peaks) and `long_min_price` (where the Longs payoff curve bottoms out). `compute_snapshot()` is the cron entry point (BTC then ETH, one row per asset per run): for each asset it first finds that asset's single nearest (soonest-to-expire) active expiry (`search_read` ordered by `expiration asc`, `limit=1`), then builds the same trade domain as the `/<instrument>/zones` route — trades since 00:00 UTC restricted to that one expiry (`("name", "=ilike", f"{asset}-%")` anchored match + `("expiration", "=", nearest_expiration)`) — and calls `options.build_zone_curves()` (see below), the exact same function the PNG route calls, so the two can never disagree on where the extrema are for the same trades. Skips creating a row for an asset if the index price fetch fails, if it has no active expiry at all, **or if zero trades match the nearest expiry's since-midnight window** — an all-zero payoffs curve has no real extremum, and writing one anyway would silently store the configured price-range floor (e.g. `dankbit.from_price`) as if it were real data.
...
- JSON API: `/api/delta-zero/<instrument>`, `/api/delta-zero-all/<asset>`, `/api/delta-zero-tomorrow/<asset>` (calendar-tomorrow's expiry, trailing-24h trades only), `/api/gamma-levels/<instrument>`, `/api/klines/<asset>`, `/api/quadrant-gamma/<asset>`, `/api/zones-extrema/<asset>` (30-day lookback of `dankbit.zones.extrema` rows, one entry per snapshot with `t`, `index_price`, `short_max_price`, `long_min_price`)
...
- `options.py` — `OptionStrat` class. Accumulates legs, then `plot()` returns a matplotlib `Figure`; `plot_zones(longs_curve, shorts_curve, index_price, title)` renders the Longs/Shorts zones chart from two separately-accumulated `OptionStrat` instances. Uses Agg backend; do not import `pyplot` here. `build_zone_curves(instrument_name, index_price, trades, from_price, to_price, steps)` is the shared Longs/Shorts curve-building + crossing-based ±$2000 zoom logic (module-level function, not a method) — used by both `chart_png_zones` and `dankbit.zones.extrema._snapshot_asset`, so the PNG chart and the cron-stored extrema can never compute different values for the same trades.
